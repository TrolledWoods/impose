...
pub const EMPTY_LOCAL: LocalHandle = LocalHandle {
    offset: 0,
    align: 1,
    size: 0,
    id: LocalId::create_raw(9999),
};
...
impl StackFrameLayout {
    ...
    pub fn create_instance_with_func_args<'a>(
        ...
        let mut instance = StackFrameInstance {
            buffer: std::pin::Pin::new(
                vec![ForceAlignment([0; STACK_FRAME_ALIGNMENT]); self.total_size]
                    .into_boxed_slice(),
            ),
            layout: self.clone(),
        };
...
impl StackFrameInstance {
    pub fn address_of_local(&self, local: LocalHandle) -> *const u8 {
        ...
        unsafe { self.buffer_ptr().add(pos) }
    }
    ...
    pub fn get_value<'a>(&'a self, value: &'a Value) -> &'a [u8] {
        match *value {
            ...
            Value::Function(ref num) => unsafe {
                std::slice::from_raw_parts(num as *const usize as *const u8, 8)
            },
            Value::Constant(ref vector) => vector.as_slice(),
        }
    }

    pub fn insert_value_into_indirect_local(&mut self, local: LocalHandle, value: &Value) {
        let pointer_pos = self.layout.local_pos(local.id);
        ...
        unsafe {
            ...
            let size = value.size();
            std::ptr::copy(self.get_value(value).as_ptr(), to, size);
        }
    }
    ...
    pub fn get_at_index<T>(&self, index: usize) -> T
    where
        T: Copy,
    {
        ...
        unsafe { *(self.buffer_ptr().add(index) as *const T) }
    }
    ...
    pub fn insert_into_index(&mut self, index: usize, data: &[u8]) {
        unsafe {
            std::ptr::copy(
                data.as_ptr(),
                (self.buffer.as_mut_ptr() as *mut u8).add(index),
                data.len(),
            );
        }
    }
