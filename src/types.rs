	...
	pub fn try_type_ast(
		...
		while self.node_id < ast.nodes.len() {
			...
			let type_kind = match node.kind {
				...
				NodeKind::Loop(_, _, break_label) => {
					Some(*ast.locals.labels.get(break_label))
				}
				NodeKind::If { condition, .. } => {
					let condition_type = ast.nodes[condition as usize].type_.unwrap();
				...
				NodeKind::IfWithElse { true_body, false_body, condition, .. } => {
					let condition_type = ast.nodes[condition as usize].type_.unwrap();
					if condition_type != BOOL_TYPE_ID {
						return error!(node,
							"If condition has to be Bool, got {}",
