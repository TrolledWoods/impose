...
pub fn get_binary_operator_intrinsic(
    operator: Operator,
    types: &Types,
    left_type: TypeId,
    right_type: TypeId,
    ...
    Some(match (operator, left_type, right_type) {
        ...
        (Operator::Sub, left, right) => match (&types.get(left).kind, &types.get(right).kind) {
...
pub fn run_intrinsic_two(intrinsic: IntrinsicKindTwo, buf: &mut u64, a: &[u8], b: &[u8]) {
    match intrinsic {
        ...
        IntrinsicKindTwo::LessF32 => {
            *buf = (f32::from_bits(buf_to_u32(a)) < f32::from_bits(buf_to_u32(b))) as u64
        }
        IntrinsicKindTwo::LessEquF32 => {
            *buf = (f32::from_bits(buf_to_u32(a)) <= f32::from_bits(buf_to_u32(b))) as u64
        }
        IntrinsicKindTwo::GreaterF32 => {
            *buf = (f32::from_bits(buf_to_u32(a)) > f32::from_bits(buf_to_u32(b))) as u64
        }
        IntrinsicKindTwo::GreaterEquF32 => {
            *buf = (f32::from_bits(buf_to_u32(a)) >= f32::from_bits(buf_to_u32(b))) as u64
        }

        IntrinsicKindTwo::LessF64 => {
            *buf = (f64::from_bits(buf_to_u64(a)) < f64::from_bits(buf_to_u64(b))) as u64
        }
        IntrinsicKindTwo::LessEquF64 => {
            *buf = (f64::from_bits(buf_to_u64(a)) <= f64::from_bits(buf_to_u64(b))) as u64
        }
        IntrinsicKindTwo::GreaterF64 => {
            *buf = (f64::from_bits(buf_to_u64(a)) > f64::from_bits(buf_to_u64(b))) as u64
        }
        IntrinsicKindTwo::GreaterEquF64 => {
            *buf = (f64::from_bits(buf_to_u64(a)) >= f64::from_bits(buf_to_u64(b))) as u64
        }
        ...
        IntrinsicKindTwo::PointerAdd { size } => {
            *buf = buf_to_u64(a).wrapping_add(buf_to_u64(b) * size)
        }
        IntrinsicKindTwo::PointerSub { size } => {
            *buf = buf_to_u64(a).wrapping_sub(buf_to_u64(b) * size)
        }
        IntrinsicKindTwo::PointerDiff { size } => {
            *buf = buf_to_u64(a).wrapping_sub(buf_to_u64(b)) / size
        }
    };
}
